...
# ⚠ 重要：本配置是【项目级】的——只对当前项目生效，
#   换一个项目 = 另一套配置（每个项目独立治理，无全局配置）。
# 使用方法：
#   1. 需要自定义哪一项，取消注释并改成你的值即可；
#   2. 未出现的键自动使用内置默认值（随工具版本更新）；
#   3. 保存文件后立即生效（对之后的新操作生效；正在进行的
#      编排 run 使用创建时的预算，不会中途改变）；
#   4. 保存后建议先验证：
#        stinky-cobbler orchestration config show --root <项目路径>
#      改错（版本不符/格式错误）会直接报错（fail-closed），
#      不会悄悄使用旧值——报错就出现在这条命令的输出里；
#   5. 本文件被删除后下次 init/scaffold 会重新生成，但绝不覆盖
#      你已经修改过的文件。
