...
function assertIssueInput(input: LeaseIssueInput): void {
  ...
  if (!Number.isSafeInteger(maxToolCalls) || maxToolCalls < 1 || maxToolCalls > MAX_MAX_TOOL_CALLS) throw leaseError("LEASE_MAX_TOOL_CALLS_INVALID", "maxToolCalls must be between 1 and 100.", { maxToolCalls });
  const expiresInMinutes = input.expiresInMinutes ?? DEFAULT_EXPIRES_IN_MINUTES;
  if (!Number.isSafeInteger(expiresInMinutes) || expiresInMinutes < 1 || expiresInMinutes > MAX_EXPIRES_IN_MINUTES) throw leaseError("LEASE_EXPIRES_IN_INVALID", "expiresInMinutes must be between 1 and 1440.", { expiresInMinutes });
