...
const identityText = z.string().min(1).max(256).regex(/^[^\0\r\n]+$/);
const workspacePath = z.string().min(1).max(4096).regex(/^[^\0\r\n]+$/);
const relativePath = z.string().min(1).max(512).regex(/^[^\0\r\n]+$/);
